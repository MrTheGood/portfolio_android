...
class ProjectViewModel : ViewModel() {
    private val _project = MutableLiveData<Resource<Project, Exception>>()
    val project: LiveData<Resource<Project, Exception>>
        get() = _project

    fun selectProject(selectedId: String) {
        _project.value = ProjectRepository.projects.value?.run {
            Resource(
                    state,
                    data?.projects?.find { it.id == selectedId },
                    error
            )
        }
    }
